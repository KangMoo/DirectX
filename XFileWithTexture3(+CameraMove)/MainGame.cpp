...
{
	_vEye = D3DXVECTOR3(2, 2, -5);
	_vLookAt =  D3DXVECTOR3(0, 0, 0);
	...
	//모델까지의 경로
	std::string fileName("Fighter/Fighter.x");
	...
	D3DXMatrixRotationY(&matRotate, D3DXToRadian(0));
...
void MainGame::Update()
{
	...
	D3DXVECTOR3 diff = _vLookAt - _vEye;
	D3DXVec3Normalize(&diff, &diff);
	D3DXMATRIX speed;

	D3DXMatrixScaling(&speed, 0.2, 0.2, 0.2);
	if (Input->Press('W'))
	{
		D3DXVec3TransformCoord(&diff, &diff, &speed);
		_vEye += diff;
		_vLookAt += diff;
	}
	if (Input->Press('A'))
	{
		D3DXVECTOR3 moveVec;
		D3DXVec3Cross(&moveVec, &diff, new D3DXVECTOR3(0, 1, 0));
		D3DXVec3Normalize(&moveVec, &moveVec);
		D3DXVec3TransformCoord(&moveVec, &moveVec, &speed);
		_vEye += moveVec;
		_vLookAt += moveVec;
	}
	if (Input->Press('S'))
	...
	{
		GetCursorPos(&_prevMoustPoint);
	}
	if (Input->Press(VK_LBUTTON))
	{
		
		POINT nMousePoint;
		GetCursorPos(&nMousePoint);
		D3DXMATRIX matRotX;
		D3DXMATRIX matRotY;
		//x축 변화량
		if (_prevMoustPoint.x != nMousePoint.x)
		{

			diff = _vLookAt - _vEye;
			D3DXMatrixRotationY(&matRotY, D3DXToRadian((float)(_prevMoustPoint.x - nMousePoint.x) / 5));
			D3DXVec3TransformCoord(&diff, &diff, &matRotY);
			_vLookAt = diff;
		}

		if (_prevMoustPoint.y != nMousePoint.y)
		{
			
			//y축 변화량
			diff = _vLookAt - _vEye;
			D3DXVECTOR2 normalizedVec = D3DXVECTOR2(diff.x, diff.z);
			D3DXVec2Normalize(&normalizedVec, &normalizedVec);
			float deg = D3DXVec2Dot(&normalizedVec, new D3DXVECTOR2(0, 1));
			
			D3DXMatrixRotationY(&matRotY, D3DXToRadian(deg));
			D3DXVec3TransformCoord(&diff, &diff, &matRotY);
			

			if ( _vLookAt.z > _vEye.z)
			{
				D3DXMatrixRotationX(&matRotX, D3DXToRadian((float)(_prevMoustPoint.y - nMousePoint.y) / 5));
				D3DXVec3TransformCoord(&diff, &diff, &matRotX);
			}
			else
			{
				D3DXMatrixRotationX(&matRotX, D3DXToRadian((float)( nMousePoint.y- _prevMoustPoint.y) / 5));
				D3DXVec3TransformCoord(&diff, &diff, &matRotX);
			}
