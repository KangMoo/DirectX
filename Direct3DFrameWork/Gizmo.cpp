...
void Gizmo::DrawLine(const D3DXVECTOR3 & start, const D3DXVECTOR3 & end, D3DXCOLOR color)
{
	...
	DEVICE->DrawPrimitiveUP		//게임 플레이가 아닌 개발할 때만 사용할 것이기 때문에 CPU사용
	(
		D3DPT_LINELIST,
		1,
		vertex,
...
void Gizmo::WorldGrid(int count)
{
	...
	{
		if (i == 0) continue;
		DrawLine(D3DXVECTOR3(-count * interval, 0, i*interval), D3DXVECTOR3(count * interval, 0, i*interval), 0xFF808080);
		DrawLine(D3DXVECTOR3(i*interval, 0, -count * interval), D3DXVECTOR3(i*interval, 0, count * interval), 0xFF808080);
	}
	DrawLine(D3DXVECTOR3(0, 0, -count * interval), D3DXVECTOR3(0, 0, count * interval), 0xFF0000FF);
	DrawLine(D3DXVECTOR3(0, -count * interval, 0), D3DXVECTOR3(0, count * interval, 0), 0xFF00FF00);
	DrawLine(D3DXVECTOR3(-count * interval, 0, 0), D3DXVECTOR3(count * interval, 0, 0), 0xFFFF0000);
}
void Gizmo::WorldGrid(int count, float interval)
{
	for (int i = -count; i <= count; i++)
	{
		if (i == 0) continue;
